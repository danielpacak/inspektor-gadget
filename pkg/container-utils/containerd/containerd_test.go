...
func TestNamespace(t *testing.T) {
	...
	c1.Start(t)
	k8sClient, err := NewContainerdClient("", nil, false)
	t.Cleanup(func() {
		k8sClient.Close()
		c1.Stop(t)
	})
	...
	c2.Start(t)
	defaultClient, err := NewContainerdClient("", &containerutilsTypes.ExtraConfig{Namespace: "default"}, false)
	t.Cleanup(func() {
		defaultClient.Close()
		c2.Stop(t)
	})
	...
	emptyClient, err := NewContainerdClient("", &containerutilsTypes.ExtraConfig{Namespace: "empty-ns"}, false)
	t.Cleanup(func() {
		emptyClient.Close()
	})
	require.Nil(t, err)
