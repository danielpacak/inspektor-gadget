...
func (m *GadgetMetadata) validateProfilers(spec *ebpf.CollectionSpec) error {
	...
	for name, profiler := range m.Profilers {
		...
		if profiler.StructValueName == "" {
			result = multierror.Append(result, fmt.Errorf("profiler %q is missing structValueName", name))
		}
		...
		_, ok = m.Structs[profiler.StructValueName]
		if !ok {
			result = multierror.Append(result, fmt.Errorf("profiler %q references unknown value struct %q", name, profiler.StructValueName))
		}
